...
class SearchResultContainer extends React.Component {
  ...
  render() {
    return (
      <div id="search-results" className={'mt-4 w-100' + (this.props.viewType === 'grid' ? ' search-result-grid' : '')}>
        {this.props.children}
      </div>
    );
  }
            ...
            <div className={(useGrid ? 'card-body' : 'd-inline py-1') + ' px-0'} style={{ overflow: 'hidden' }}>
              <div className={'card-text' + (useGrid ? '' : ' d-inline')}>
                ...
                >
                  ...
                  {this.props.showTaxaDetail === 'on' && <span className="author"> ({this.props.author})</span>}
                  {!useGrid && this.props.commonName.length > 0 ? (
                  ...
                  <div className="vouchers">
                    ...
                    {this.props.vouchers
                      .map((voucher) => {
                        return (
                          <a
                            href={this.props.clientRoot + '/collections/individual/index.php?occid=' + voucher.occid}
                            target="_blank"
                            key={voucher.occid}
                            className={'voucher'}
                            rel="noreferrer"
                      ...
                      <a
                        ...
                        onClick={() =>
                          this.saveEditSPP({
                            ...
                            value: this.props.tid,
                            section: 'spp',
                            notes: this.state.updatedData['notes'],
                          })
                        }
                      >
                        Save
                      </a>
                      <a type="button" className="btn btn-primary" onClick={() => this.toggleEditingNotes()}>
                        Cancel
                      </a>
                    </div>
                  </div>
...
function VendorUploadContainer(props) {
  ...
  return (
    <div id="search-results" className={'vendor-upload-preview'}>
      {<Searching clientRoot={props.clientRoot} isSearching={props.isSearching} />}
              ...
              return (
                <tr key={index}>
                  ...
                  <td className="feedback">
                    {result.feedback && result.feedback.map((t) => <span key={t}>{t}</span>)}
                  </td>
                </tr>
              );
            })}
