...
class IdentifyApp extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([dataPromise, glossaryPromise]).then(([data, glossary]) =>
      this.addGlossaryToChars(data.characteristics, glossary),
    );
  }
  updateExportUrls() {
  ...
  onSliderChanged(sliderState, range) {
    ...
    if (range[0] == min && range[1] == max) {
      ...
      filters.sliders[sliderState.cid] = {
        range: sliderState.range,
        label: sliderState.label,
        units: sliderState.units,
        step: sliderState.step,
        originalStates: sliderState.states,
      };
    }
          ...
          <div className="row pb-2">
            <div className="col-9 copy">
              ...
              {this.state.authors.length > 0 && (
                <p className="authors">
                  ...
                  <span className="authors-content" dangerouslySetInnerHTML={{ __html: this.state.authors }} />
                </p>
              )}
              {this.state.abstract.length > 0 && this.state.displayAbstract == 'default' && (
                <div>
                  <p className="abstract">
                    ...
                    <span className="abstract-content" dangerouslySetInnerHTML={{ __html: shortAbstract }} />
                  </p>
              ...
              {this.state.abstract.length > 0 && this.state.displayAbstract == 'expanded' && (
                <div>
                  <p className="abstract">
                    <strong>Abstract:</strong>{' '}
                    <span className="abstract-content" dangerouslySetInnerHTML={{ __html: this.state.abstract }} />
                  </p>
            ...
            <div className="col-3 text-right mt-3 map">
              {this.state.googleMapUrl.length > 0 && (
                <a
                  href={this.props.clientRoot + '/checklists/checklistmap.php?clid=' + this.getClid()}
                  target="_blank"
                  rel="noreferrer"
                >
                  <img
                ...
                <div className="col">
                  <div className="identify-header inventory-header">
                    <div className="current-wrapper">
                      <div className="btn btn-primary current-button">
                  ...
                  {this.getDynclid() > 0 &&
                    ...
                    this.state.isLoading == false && (
                      <p>
                        <strong>No results found:</strong> Your dynamic checklist may have expired.{' '}
                        <a href={this.props.clientRoot + '/checklists/dynamicmap.php?interface=key'}>Try again?</a>
                      </p>
                    )}
                  {this.state.searchResults.taxonSort.length > 0 ? (
          ...
          <FilterModal show={this.state.showFilterModal}>
            <div className="modal-filter-content">
              <div className="filter-count">
                {filterCount} filter{filterCount > 1 ? 's' : ''} chosen
              </div>
