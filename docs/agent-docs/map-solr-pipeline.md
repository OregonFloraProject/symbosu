...
## Taxon report download mode (`download=csv|docx`)

When the request carries `download=csv` or `download=docx` (POST or GET), `spatial/rpc/solrSearch.php:80-114` takes a download branch instead of the normal GeoJSON search. It validates the format, then calls `fetchDistinctTidInterpreted($searchParams)` (`spatial/shared/solrSearchHelpers.php:481`) which reuses `buildTaxaParams`/`buildTextParams`/`buildGeographyParams` to build the same `$solrQ`/`$solrGeoQ` (with `$solrManager->checkQuerySecurity()` applied), pages `tidinterpreted` only (`fl=tidinterpreted`, capped at `MAX_RECORD_COUNT = 20000`), filters to numeric `>0`, deduplicates and sorts. If empty it returns `{"error": true, "message": "No taxa found for current search"}`. Otherwise `classes/DynamicChecklistManager.php:114` `createDynamicChecklistFromTids($tids)` inserts an ephemeral `fmdynamicchecklists` row (`name="Map Taxon Report"`, 7-day `expiration`) and `INSERT IGNORE INTO fmdyncltaxalink (dynclid, tid)` for the distinct tids, returning `dynclid`. The endpoint then includes `ident/shared/checklistApi.php`, calls `get_data(['dynclid' => $dynclid])` and `array_walk_recursive(...,'cleanWindowsRecursive')`, and streams `exportChecklistToCSV($result)` for `csv` or `exportChecklistToWord($result)` for `docx` (`checklists/checklistexport.php`); the `docx` download value maps to the `word` export which sets Word headers. The same error envelope is used for invalid format or checklist-creation failure.
