...
foreach($customFieldArr as $k => $v){
	$customFieldArr[$k] = defined(strtoupper($k).'LABEL') ? constant(strtoupper($k).'LABEL') : $v;
}

// Sort array by the values (displayed field names), case insensitive
asort($customFieldArr, SORT_NATURAL | SORT_FLAG_CASE);
	...
	<fieldset style="padding:0.5rem; position: relative">
		...
		<div style="float: right">
			...
			<form name="download" action="../download/index.php" method="post" target="downloadpopup" onsubmit="window.open('', 'downloadpopup', 'left=100,top=50,width=900,height=700'); f.target = 'downloadpopup';" style="display:inline-flex;">
				<button  id="downloadButton" type="submit" class="icon-button" aria-label="<?= $LANG['DOWNLOAD_SPECIMEN_DATA'] . $LANG['OPENS_NEW_TAB'] ?>" title="<?= $LANG['DOWNLOAD_SPECIMEN_DATA'] ?>" style="width: 36px;height: 36px;padding-top:8px">
		...
		<form name="queryform" action="<?php echo $_SERVER['SCRIPT_NAME']; ?>" method="post" onsubmit="return verifyQueryForm(this)">
			...
			if(!$crowdSourceMode){
				...
				<div class="fieldGroupDiv">
					<div class="fieldDiv" title="<?php echo $LANG['WILD_EXPLAIN']; ?>">
						<label for="q_recordedby"><?php echo (defined('RECORDEDBYLABEL') ? RECORDEDBYLABEL : $LANG['COLLECTOR']); ?>:</label>
					...
					<div class="fieldDiv" title="<?php echo $LANG['SEPARATE_RANGES']; ?>">
						<label for="q_recordnumber"><?php echo (defined('RECORDNUMBERLABEL') ? RECORDNUMBERLABEL : $LANG['NUMBER']); ?>:</label>
						<input type="text" name="q_recordnumber" id="q_recordnumber" value="<?php echo $qRecordNumber; ?>" style="width:120px;" onchange="setOrderBy(this)" />
							<label  title="<?php echo (defined('EVENTDATELABEL') ? EVENTDATELABEL : $LANG['ENTER_RANGES']); ?>" for="q_eventdate"><?php echo $LANG['DATE']; ?>:</label>
			...
			<div class="fieldGroupDiv">
				<div class="fieldDiv" title="<?php echo $LANG['SEPARATE_RANGES']; ?>">
					<label for="q_catalognumber"><?php echo (defined('CATALOGNUMBERLABEL') ? CATALOGNUMBERLABEL : $LANG['CAT_NUM']); ?>:</label>
				...
				if($crowdSourceMode){
					...
					?>
					<div class="fieldDiv " title="<?php echo $LANG['SEPARATE_RANGES']; ?>">
						<label for="q_othercatalognumbers"><?php echo (defined('OTHERCATALOGNUMBERSLABEL') ? OTHERCATALOGNUMBERSLABEL : $LANG['OTHER_CAT_NUMS']); ?>:</label>
			...
			if(!$crowdSourceMode){
				...
				<div class="fieldGroupDiv" <?php echo ($isGenObs?'style="display:none"':''); ?>>
					<div class="fieldDiv">
						...
						<input type="text" name="q_user" id="q_recordenteredby" value="<?php echo $qUser; ?>" style="max-width:70px;" onchange="setOrderBy(this)" />
					</div>
				...
				<div class="fieldGroupDiv">
					<div class="fieldDiv">
						<label for="q_processingstatus"> <?php echo (defined('PROCESSINGSTATUSLABEL') ? PROCESSINGSTATUSLABEL : $LANG['PROC_STATUS']); ?>: </label>
			...
			<div>
				...
				<section class="fieldGroupDiv">
					...
					<div style="margin-left: 0;">
						<button id="displayTableButton" name="submitaction" type="submit" onclick="submitQueryTable(this.form)" ><?php echo $LANG['DISPLAY_TABLE']; ?></button>
					</div>
					<div style="margin-left: 0;">
				...
				<section class="fieldGroupDiv">
					<div class="" style="margin-left: 0;">
						...
						<select name="orderby" id="orderby">
							<option value=""></option>
							<option value="recordedby" <?php echo ($qOrderBy=='recordedby'?'SELECTED':''); ?>><?php echo (defined('RECORDEDBYLABEL') ? RECORDEDBYLABEL : $LANG['COLLECTOR']); ?></option>
							<option value="recordnumber" <?php echo ($qOrderBy=='recordnumber'?'SELECTED':''); ?>><?php echo (defined('RECORDNUMBERLABEL') ? RECORDNUMBERLABEL : $LANG['NUMBER']); ?></option>
							<option value="eventdate" <?php echo ($qOrderBy=='eventdate'?'SELECTED':''); ?>><?php echo (defined('EVENTDATELABEL') ? EVENTDATELABEL : $LANG['DATE']); ?></option>
							<option value="catalognumber" <?php echo ($qOrderBy=='catalognumber'?'SELECTED':''); ?>><?php echo (defined('CATALOGNUMBERLABEL') ? CATALOGNUMBERLABEL : $LANG['CAT_NUM']); ?></option>
							...
							<option value="processingstatus" <?php echo ($qOrderBy=='processingstatus'?'SELECTED':''); ?>><?php echo (defined('PROCESSINGSTATUSLABEL') ? PROCESSINGSTATUSLABEL : $LANG['PROC_STATUS']); ?></option>
							<option value="sciname" <?php echo ($qOrderBy=='sciname'?'SELECTED':''); ?>><?php echo (defined('SCIENTIFICNAMELABEL') ? SCIENTIFICNAMELABEL : $LANG['SCI_NAME']); ?></option>
							<option value="family" <?php echo ($qOrderBy=='family'?'SELECTED':''); ?>><?php echo (defined('FAMILYLABEL') ? FAMILYLABEL : $LANG['FAMILY']); ?></option>
							<option value="country" <?php echo ($qOrderBy=='country'?'SELECTED':''); ?>><?php echo (defined('COUNTRYLABEL') ? COUNTRYLABEL : $LANG['COUNTRY']); ?></option>
							<option value="stateprovince" <?php echo ($qOrderBy=='stateprovince'?'SELECTED':''); ?>><?php echo (defined('STATEPROVINCELABEL') ? STATEPROVINCELABEL : $LANG['STATE_PROVINCE']); ?></option>
							<option value="county" <?php echo ($qOrderBy=='county'?'SELECTED':''); ?>><?php echo (defined('COUNTYLABEL') ? COUNTYLABEL : $LANG['COUNTY']); ?></option>
							<option value="municipality" <?php echo ($qOrderBy=='municipality'?'SELECTED':''); ?>><?php echo (defined('MUNICIPALITYLABEL') ? MUNICIPALITYLABEL : $LANG['MUNICIPALITY']); ?></option>
							<option value="locationid" <?php echo ($qOrderBy=='locationid'?'SELECTED':''); ?>><?php echo (defined('LOCATIONIDLABEL') ? LOCATIONIDLABEL : $LANG['LOCATION_ID']); ?></option>
							<option value="locality" <?php echo ($qOrderBy=='locality'?'SELECTED':''); ?>><?php echo (defined('LOCALITYLABEL') ? LOCALITYLABEL : $LANG['LOCALITY']); ?></option>
							<option value="decimallatitude" <?php echo ($qOrderBy=='decimallatitude'?'SELECTED':''); ?>><?php echo (defined('DECIMALLATITUDELABEL') ? DECIMALLATITUDELABEL : $LANG['DEC_LAT']); ?></option>
							<option value="decimallongitude" <?php echo ($qOrderBy=='decimallongitude'?'SELECTED':''); ?>><?php echo (defined('DECIMALLATITUDELABEL') ? DECIMALLATITUDELABEL : $LANG['DEC_LONG']); ?></option>
