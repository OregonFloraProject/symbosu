	...
	foreach($labelArr as $occid => $occArr){
		...
		if($barcodeOnly){
			if($occArr['catalognumber']){
				...
				imagepng($bc,$TEMP_DIR_ROOT.'/report/'.$ses_id.$occArr['catalognumber'].'.png');
				$textrun->addImage($TEMP_DIR_ROOT.'/report/'.$ses_id.$occArr['catalognumber'].'.png', array('align'=>'center'));
				imagedestroy($bc);
			}
		}
		else{
			...
			for($i = 0;$i < $dupCnt;$i++){
				...
				if($useBarcode && $occArr['catalognumber']){
					...
					$bc = $bcObj->draw(strtoupper($occArr['catalognumber']),"Code39","png",false,40);
					imagepng($bc,$TEMP_DIR_ROOT.'/report/'.$ses_id.$occArr['catalognumber'].'.png');
					$textrun->addImage($TEMP_DIR_ROOT.'/report/'.$ses_id.$occArr['catalognumber'].'.png', array('align'=>'center','marginTop'=>0.15625));
				...
				if($useSymbBarcode){
					...
					imagepng($bc,$TEMP_DIR_ROOT.'/report/'.$ses_id.$occid.'.png');
					$textrun->addImage($TEMP_DIR_ROOT.'/report/'.$ses_id.$occid.'.png', array('align'=>'center','marginTop'=>0.104166667));
					if($occArr['catalognumber']){
						$textrun->addTextBreak(1);
						$textrun->addText(htmlspecialchars($occArr['catalognumber']),'otherFont');
					}
...
$targetFile = $TEMP_DIR_ROOT.'/report/'.$PARAMS_ARR['un'].'_'.date('Ymd').'_labels_'.$ses_id.'.docx';
...
readfile($targetFile);
$files = glob($TEMP_DIR_ROOT.'/report/*');
