...
#  ADD COLUMN `recordID` VARCHAR(45) NULL AFTER `dwcTermJson`;
  CHANGE COLUMN `recordID` `recordID` VARCHAR(45) NULL DEFAULT NULL;
...
ALTER TABLE `specprocessorprojects` 
  DROP COLUMN `customStoredProcedure`;
...
ALTER TABLE `taxalinks` 
  ...
  ADD UNIQUE INDEX `UQ_taxaLinks_tid_url` (`tid` ASC, `url`(255) ASC) ;

# Delete duplicates
DELETE t1 FROM taxalinks t1 INNER JOIN taxalinks t2 WHERE t1.tlid > t2.tlid AND t1.tid = t2.tid AND t1.url = t2.url; 
...
# This will be used initially for the rare plant guide.
