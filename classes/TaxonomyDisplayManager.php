...
class TaxonomyDisplayManager extends Manager{
	...
	private function setTaxa(){
		...
		$sql .= ($this->oregonVascPlant ? 'INNER JOIN `fmchklsttaxalink` as cl ON t.tid = cl.tid ' : '');
		...
		if($this->taxaArr){
			...
			$sql2 = 'SELECT t.tid, t.sciname, t.cultivarepithet, t.tradename, t.author, t.rankid, ts.parenttid, IF(cl.clid = 1, cl.clid, NULL) as clid
			...
			$sql2 .= 'LEFT JOIN `fmchklsttaxalink` as cl ON t.tid = cl.tid ';
			...
			$sql2 .= ($this->oregonVascPlant ? 'AND (cl.clid = 1) ' : '');
	...
	private function echoTaxonArray($node){
		if($node){
			...
			foreach($node as $key => $value){
				...
				if($this->isEditor){
					echo ' <a href="taxoneditor.php?tid=' . $key . '" target="_blank"><img class="icon-image" src="../../images/edit.png" alt="Edit"></a> <a href="../profile/tpeditor.php?tid=' . $key . '" target="_blank"><img class="icon-image" src="../../images/info2.png" alt="Edit Taxon Profile" /></a>';
				}
				if($this->limitToOccurrences){
