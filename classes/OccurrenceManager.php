...
class OccurrenceManager extends OccurrenceTaxaManager {
	...
	protected function setSqlWhere(){
		...
		if($countryTermArr) $sqlWhereConditional .= 'AND ('.implode(' OR ',$countryTermArr).') ';
		...
		if(array_key_exists('hasimages', $this->searchTermArr)){
			if($this->searchTermArr['hasimages']) {
				$sqlWhereConditional .= 'AND (o.occid IN(SELECT occid FROM media where mediaType = "image")) ';

			} else {
				$sqlWhereConditional .= 'AND (o.occid NOT IN(SELECT occid FROM media where mediaType = "image" and occid IS NOT NULL)) ';
			}
