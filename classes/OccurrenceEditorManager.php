...
class OccurrenceEditorManager {
	...
	protected function setOccurArr($start = 0, $limit = 0) {
		...
		if($sqlFrag){
			...
			if(!$this->occid) {
				$sql = 'SELECT DISTINCT o.occid, o.collid, lastuser.username as recordmodifiedby, o.'.implode(',o.',array_keys($this->fieldArr['omoccurrences'])) . ',paleo.' . implode(',paleo.',($this->fieldArr['omoccurpaleo'])).', o.datelastmodified FROM omoccurrences o '.$sqlFrag;
			} else {
	...
	private function addTableJoins(&$sql, $last_modified = FALSE){
		...
		if(array_key_exists('mb',$this->qryArr)){
			...
			$sql .= 'LEFT JOIN users as user ON edits.uid = user.uid ';
		}
