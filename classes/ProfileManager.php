...
class ProfileManager extends Manager{
	 ...
	 *
	 * The following four helper functions get and set these fields and are used by
	 * profile/rpc/api.php.
	 */
	public function populateRareSpeciesAccessFields() {
		...
		$isExpiring = false;
		if ($this->uid) {
			$sqlStr = 'SELECT u.notes, u.accessrRights, r.initialTimestamp ' .
				'FROM users u ' .
			...
			if ($r = $rs->fetch_object()) {
				...
				if ($r->initialTimestamp && preg_match('/expiration=(\d+);/', $r->notes, $match) && (int)$match[1]) {
