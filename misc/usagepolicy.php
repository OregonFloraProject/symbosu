		...
		<div id="innertext">
			...
			<div style="margin:10px">
				...
				<div style="margin:10px;">
					...
					echo 'https://'.$_SERVER['HTTP_HOST'].$clientRoot.'. ';#.(substr($clientRoot,-1)=='/'?'':'/')
				...
				<div style="margin:10px;">
					...
					<?php echo 'https://'.$_SERVER['HTTP_HOST'].$clientRoot; ?>, YYYY-MM-DD)<br/><br/>
					<b>For example:</b><br/>
					...
					(Accessed through <?php echo $defaultTitle; ?> Data Portal, 
					<?php echo 'https://'.$_SERVER['HTTP_HOST'].$clientRoot.', '.date('Y-m-d').')'; ?>
				</div>
			</div>
			<div>
			</div>
		    ...
		    <div style="margin:10px;">
				<ul>
					...
					<li>
						<?php echo $defaultTitle; ?> asks that users not redistribute data obtained from this site without permission of data owners. 
		    ...
		    <div style="margin:15px;">
				...
				of taxa that no longer occur within the area. 
				By making these specimens available on the web as images, their availability and 
				value improves without an increase in inadvertent damage caused by use. If you are considering making specimens, note that collecting normally requires 
				...
				regional institution that manages a publicly accessible collection.
			</div> 
		</div>
		<?php
